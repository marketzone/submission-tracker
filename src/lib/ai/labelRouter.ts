...
// This module maps workbookTitle to a single deliverable type so the correct
// pattern row(s) are injected — not all of them.
//
// Routing priority:
//   1. Canonical-name exact match (new dropdown submissions) — fast path, no ambiguity.
//   2. Keyword heuristic (legacy free-text submissions) — existing logic unchanged.
//
...
import { CANONICAL_TITLES } from "@/lib/deliverables"
...
// ── Canonical name maps ───────────────────────────────────────────────────────
// New dropdown submissions produce exact canonical names — route without ambiguity.

const CANONICAL_W3: Record<string, DeliverableType> = {
  "Ad Copy Scripts (3 Angles)": "ad_copy",
  "Ad Video Script": "ad_video",
}

const CANONICAL_W6: Record<string, DeliverableType> = {
  "Webinar / Workshop / Challenge Presentation": "webinar",
  "Core-Offer Sales Page": "sales_copy",
}

// ── Keyword sets (legacy fallback) ────────────────────────────────────────────
...
export function routeByLabel(weekNumber: number, workbookTitle: string): LabelRouteResult {
  ...
  if (weekNumber === 3) {
    // Canonical exact match — new dropdown submission
    if (CANONICAL_TITLES.has(t) && CANONICAL_W3[t]) {
      return { outcome: "matched", deliverableType: CANONICAL_W3[t] }
    }

    // Legacy keyword fallback
    const isVideo        = W3_VIDEO_RE.test(t)
    const isExplicitCopy = W3_ADCOPY_RE.test(t)
    const isScript       = W3_SCRIPT_RE.test(t)
  ...
  if (weekNumber === 6) {
    // Canonical exact match — new dropdown submission
    if (CANONICAL_TITLES.has(t) && CANONICAL_W6[t]) {
      return { outcome: "matched", deliverableType: CANONICAL_W6[t] }
    }
